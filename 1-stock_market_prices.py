...
- prices is a list of numbers such that each element n is a reasonably sized, positive number, something between 0 and a million.
- The function returns a number x that is 0 or greater.
- You may buy and sell at most once each and you must buy before you sell.
- You COULD buy and sell on the same day, but that would yield zero profit.
...
def get_max_profit(prices):
    ...
    This must return the maximum possible profit per share as a single number.
    If no profit is possible, return 0.
    """
    raise NotImplementedError("get_max_profit must be implemented by the candidate")

print('Answer: ', get_max_profit(prices)) # Should output `70`
...
def test(fn):
    ...
    for t in tests:
        ...
        try:
          ...
          print("get_max_profit not implemented; aborting tests.")
          return
        if result != answer:
